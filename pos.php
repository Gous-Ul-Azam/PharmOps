					...
					<div class="col-md-12">
						<div class="row">
							...
							<div class="form-group col-md-5">
								...
								<select name="payment_type" id="payment_type" class="custom-select browser-default select2">
								...
								<tbody>
									...
									?>
											<tr class="item-row">
												...
												<td>
													<input type="number" min="1" step="any" name="qty[]" value="<?php echo $row['qty'] ?>" class="qty text-right">
												</td>
												<td>
													<input type="hidden" min="1" step="any" name="price[]" value="<?php echo $row['price'] ?>" class="price text-right">
													<p class="text-right"><?php echo $row['price'] ?></p>
												</td>
												<td>
													<input type="number" min="1" step="any" name="discount[]" value="<?php echo $row['discount'] ?>" class="discount text-right">
												</td>
												<td>
...
<div id="tr_clone">
	<table>
		<tr class="item-row">
			...
			<td>
				<input type="number" min="1" step="any" name="qty[]" value="" class="text-right qty">
			</td>
			<td>
				<input type="hidden" min="1" step="any" name="price[]" value="" class="text-right price" readonly="">
				<p class="price text-right">0</p>
			</td>
			
			<td>
				<input type="number" min="1" step="any" name="discount[]" value="<?php echo $row['discount'] ?>" value="0" class="discount text-right">
			</td>
			<td>
				<p class="amount text-right"></p>
			</td>
...
<script>
	...
	function calculate_total() {
		...
		$('#list tbody').find('.item-row').each(function() {
			...
			console.log("bfr",amount);
			amount = amount - (amount * (parseFloat(discount)/100));
			console.log("aft",amount);
				...
				if (resp > 0) {
					...
					setTimeout(function() {
						...
						<?php if(isset($_GET['id'])){
							echo "location='index.php?page=sales';";
						}else{
	...
	$(document).on('keyup','[name="qty[]"],[name="price[]"],[name="discount[]"]',function() {
						calculate_total()
					})
</script>